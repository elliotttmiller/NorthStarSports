...
import { formatOdds, formatSpreadLine, formatTotalLine } from '@/lib/formatters';
...
export const ProfessionalGameRow = memo(({ game, isFirstInGroup, showTime }: ProfessionalGameRowProps) => {
  ...
  const getBetId = useCallback((betType: string, selection: string) => {
    return `${game.id}-${betType}-${selection}`;
  }, [game.id]);

  const isBetInSlip = useCallback((betType: string, selection: string) => {
    const betId = getBetId(betType, selection);
    return Array.isArray(betSlip?.bets) && betSlip.bets.some((b) => b.id === betId);
  }, [betSlip, getBetId]);

  const handleBetClick = (betType: 'spread' | 'total' | 'moneyline', selection: 'away' | 'home' | 'over' | 'under', e?: React.MouseEvent) => {
    ...
    switch (betType) {
      case 'spread':
        odds = selection === 'away' ? game.odds.spread.away.odds : game.odds.spread.home.odds;
        line = selection === 'away' ? game.odds.spread.away.line : game.odds.spread.home.line;
        break;
      case 'total':
        odds = selection === 'over' ? game.odds.total.over?.odds || 0 : game.odds.total.under?.odds || 0;
        line = selection === 'over' ? game.odds.total.over?.line : game.odds.total.under?.line;
        break;
      case 'moneyline':
        odds = selection === 'away' ? game.odds.moneyline.away.odds : game.odds.moneyline.home.odds;
        line = undefined;
        break;
      default:
        return;
    ...
    // Provide feedback
    const teamName = selection === 'away' ? game.awayTeam.name : game.homeTeam.name;
      ...
      <div className="grid grid-cols-[80px_1fr_120px_120px_120px_50px] gap-4 items-center py-2 px-4 min-h-[60px]">
        ...
        <div className="space-y-1">
          ...
          >
            ...
            >
              {formatSpreadLine(game.odds.spread.away.line || 0)} {formatOdds(game.odds.spread.away.odds)}
            </Button>
          </motion.div>
          {/* Home Spread */}
          <motion.div 
          ...
          >
            ...
            >
              {formatSpreadLine(game.odds.spread.home.line || 0)} {formatOdds(game.odds.spread.home.odds)}
            </Button>
          </motion.div>
        </div>
        ...
        <div className="space-y-1">
          ...
          >
            ...
            >
              O{formatTotalLine(game.odds.total.over?.line || 0)} {formatOdds(game.odds.total.over?.odds || 0)}
            </Button>
          </motion.div>
          {/* Under */}
          <motion.div 
          ...
          >
            ...
            >
              U{formatTotalLine(game.odds.total.under?.line || 0)} {formatOdds(game.odds.total.under?.odds || 0)}
            </Button>
          </motion.div>
        </div>
