  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className="h-full bg-muted/10 flex flex-col overflow-hidden z-40"
        style={{ '--bet-slip-panel-width': '340px' } as React.CSSProperties}
      >
        <motion.div 
          className="p-4 border-b border-border flex-shrink-0"
          initial={{ opacity: 0, y: -10 }}
                      ...
                      >
                        <Card className="border border-accent/30 shadow-sm ring-1 ring-accent/15 hover:border-accent/50 bg-card/50 transition-colors duration-200">
                          <CardContent className="p-4">
                            ...
                            <div className="space-y-3">
                              <div className="flex items-center justify-between">
                                <span className="text-sm font-semibold text-foreground">Total Stake:</span>
                                <Input
                                  type="number"
                                  min="0"
                                  step="0.01"
                              ...
                              <div className="flex items-center justify-between text-sm bg-gradient-to-r from-secondary/15 to-secondary/25 rounded-lg p-3 border border-border/30">
                                ...
                                <span className="font-bold text-accent">{formatOdds(betSlip.totalOdds)}</span>
                              </div>
                            </div>
                          </CardContent>
                        </Card>
        ...
        >
          <div style={{ gap: 'var(--fluid-gap)' }} className="flex flex-col">
            <div className="flex items-center justify-between" style={{ fontSize: 'var(--fluid-lg)' }}>
              ...
              <span className="font-semibold text-card-foreground" style={{ fontSize: 'var(--fluid-xl)' }}>${betSlip.totalStake.toFixed(2)}</span>
            </div>
            <div className="flex items-center justify-between" style={{ fontSize: 'var(--fluid-lg)' }}>
              <span className="text-muted-foreground">Potential Payout:</span>
              <span className="font-semibold text-accent" style={{ fontSize: 'var(--fluid-xl)' }}>${betSlip.totalPayout.toFixed(2)}</span>
            </div>
            {betSlip.totalPayout > betSlip.totalStake && (
              <div className="flex items-center justify-between" style={{ fontSize: 'var(--fluid-base)' }}>
                <span className="text-muted-foreground">Profit:</span>
                <span className="font-medium text-[color:var(--color-win)]" style={{ fontSize: 'var(--fluid-lg)' }}>
