...
export function GameCard({ game, className, compact = false }: GameCardProps) {
  ...
  const handleBetClick = useCallback((
    e: React.MouseEvent,
    betType: 'spread' | 'moneyline' | 'total' | 'period_winner' | 'quarter_winner' | 'half_winner',
    ...
    e.stopPropagation()
    addBet(game, betType, selection, odds, line, undefined, periodOrQuarterOrHalf)
            ...
            <div className="flex flex-col flex-1 min-w-0 w-full md:w-auto gap-6">
              ...
              <div className="flex flex-col gap-2 pb-2 border-b border-border/20 last:border-b-0 last:pb-0">
                <div className="flex flex-row items-center justify-between w-full">
                  ...
                  <div className="flex flex-row items-center gap-2 text-xs text-muted-foreground min-w-[90px] justify-end">
                    ...
                    <Badge variant="secondary" className="ml-2 px-2 py-0.5 text-xs font-medium bg-muted/40">
                      {game.leagueId}
                    </Badge>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2 pt-2">
                ...
                <div className="flex flex-row gap-4 mt-2 w-full">
                  ...
                  >
                    Under {formatTotalLine(game.odds.total.under?.line || 47.5)}
                  </Button>
                </div>
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2 mb-2 auto-formatter">
                  ...
                  {game.leagueId === 'NHL' && (() => {
                    ...
                    return [1,2,3].map(num => {
                      const p = periodWinners[`${num}st`] || periodWinners[`${num}nd`] || periodWinners[`${num}rd`];
                      return p ? (
                        <Button key={num} variant="ghost" size="sm" className="h-7 px-2 text-xs rounded bg-muted/10 border border-border/40 hover:bg-accent/10 transition-colors whitespace-nowrap" onClick={e => {e.stopPropagation(); handleBetClick(e, 'period_winner', 'home', p.odds || -110, undefined, `${num}${num===1?'st':num===2?'nd':'rd'}`)}}>{`${num}st Period Winner`}</Button>
                      ) : null;
                    });
                  })()}
                  {(game.leagueId === 'NFL' || game.leagueId === 'NBA' || game.leagueId === 'NCAAF' || game.leagueId === 'NCAAB') && (() => {
                    ...
                    return [
                      ...[1,2,3,4].map(num => {
                        const q = quarterWinners[`${num}st`] || quarterWinners[`${num}nd`] || quarterWinners[`${num}rd`] || quarterWinners[`${num}th`];
                        return q ? (
                          <Button key={`q${num}`} variant="ghost" size="sm" className="h-7 px-2 text-xs rounded bg-muted/10 border border-border/40 hover:bg-accent/10 transition-colors whitespace-nowrap" onClick={e => {e.stopPropagation(); handleBetClick(e, 'quarter_winner', 'home', q.odds || -110, undefined, `${num}${num===1?'st':num===2?'nd':num===3?'rd':'th'}`)}}>{`${num}${num===1?'st':num===2?'nd':num===3?'rd':'th'} Qtr Winner`}</Button>
                        ) : null;
                      }),
                      ...[1,2].map(num => {
                        const h = halfWinners[`${num}st`] || halfWinners[`${num}nd`];
                        return h ? (
                          <Button key={`h${num}`} variant="ghost" size="sm" className="h-7 px-2 text-xs rounded bg-muted/10 border border-border/40 hover:bg-accent/10 transition-colors whitespace-nowrap" onClick={e => {e.stopPropagation(); handleBetClick(e, 'half_winner', 'home', h.odds || -110, undefined, `${num}${num===1?'st':'nd'}`)}}>{`${num}${num===1?'st':'nd'} Half Winner`}</Button>
                        ) : null;
                      })
                    ];
                  })()}
