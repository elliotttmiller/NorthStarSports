// Migrated from src/pages/LivePage.tsx
import { useEffect, useState } from "react";
import { AnimatePresence } from "framer-motion";
import { getLiveGames, getSports } from "@/services/mockApi";
...
export default function LivePage() {
  ...
  const [sports, setSports] = useState<Array<{ id: string; leagues: Array<{ id: string }> }>>([]);
  ...
  if (selectedSport) {
    const sportObj = sports.find((s) => s.id.toLowerCase() === selectedSport.toLowerCase());
    if (sportObj) {
      const leagueIds = sportObj.leagues.map((l) => l.id.toLowerCase());
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        <div className="universal-responsive-container scrollbar-hide min-h-screen">
