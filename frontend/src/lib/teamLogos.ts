 ...
 */
export function getTeam(identifier: string, league?: 'nfl' | 'nba'): TeamLogoConfig | null {
  const id = identifier.toLowerCase()
  
  // If league is specified, search only within that league
  const teamsToSearch = league ? getTeamsByLeague(league) : Object.values(ALL_TEAMS)
  ...
  const teamByAbbr = teamsToSearch.find(team => 
    team.abbreviation.toLowerCase() === id
  )
  if (teamByAbbr) return teamByAbbr
  
  // Try name match
  const teamByName = teamsToSearch.find(team => 
 ...
 */
export function getLogoUrl(team: TeamLogoConfig, format: 'svg' | 'png' = 'svg'): string {
  ...
  const logoPath = team.logoPath[format] || team.logoPath.png || team.logoPath.svg
