...
// BetSlip operations
export async function getActiveBetSlip(req: Request, res: Response, next: NextFunction): Promise<void> {
  const userId = req.params.userId;
  if (!userId) {
    logWarn('Missing userId parameter');
    res.status(400).json(error('Missing userId parameter', 400));
...
export async function setActiveBetSlip(req: Request, res: Response, next: NextFunction): Promise<void> {
  const userId = req.params.userId;
  if (!userId) {
    logWarn('Missing userId parameter');
    res.status(400).json(error('Missing userId parameter', 400));
...
export async function getBetSlipHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
  const userId = req.params.userId;
  if (!userId) {
    logWarn('Missing userId parameter');
    res.status(400).json(error('Missing userId parameter', 400));
...
export async function addBetSlipToHistory(req: Request, res: Response, next: NextFunction): Promise<void> {
  const userId = req.params.userId;
  if (!userId) {
    logWarn('Missing userId parameter');
    res.status(400).json(error('Missing userId parameter', 400));
  ...
  try {
    ...
    const currentHistory = await kvService.get(`betslip:${userId}:history`) || [];
    const updatedHistory = Array.isArray(currentHistory) ? [...currentHistory, betSlipId] : [betSlipId];
