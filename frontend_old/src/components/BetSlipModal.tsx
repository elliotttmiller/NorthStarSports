...
export const BetSlipModal = memo(function BetSlipModal() {
  const { betSlip, removeBet, updateStake, setBetType, clearBetSlip } =
    useBetSlipStore();
  const { addBet, refreshBets } = useBetsStore();
  const { navigation, setIsBetSlipOpen } = useNavigationStore();
      ...
      if (betSlip.betType === "single") {
        ...
        const parlayBet = {
          ...
          game: {
            ...
            awayTeam: {
              name: `${betSlip.bets.length} Picks`,
              shortName: `${betSlip.bets.length}P`,
            },
            leagueId: "PARLAY",
          } as any,
                    ...
                    >
                      {isPlacing ? (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span>
                            Place{" "}
                            {betSlip.betType === "single" ? "Bets" : "Parlay"}
                          </span>
                        </div>
                      )}
                    </Button>
